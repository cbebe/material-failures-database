 ...
 * @return { String } sql
 */
export const generateSql = (tablename: string, id: string) => {
  return `SELECT * 
            FROM ${tablename} 
            WHERE object_id = ${id}`;
};
 ...
 */
// TODO: FIND SHAPE LATER
export const formatByHeader = (table: any) => {
...
const formatByRow = (rows) => {
  const { errorMessages, excludedColumns } = constants;
 ...
 * @return { Object Literal } response formatted table
 */
export const formatByPromptVisibility = (table) => {
  const { headers, excludedColumns } = constants;
...
export const formatByValueVisibility = (table: any) => {
  ...
  for (let column in response) {
    if (
      response[column] === constants.visibility.off ||
