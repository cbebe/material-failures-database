...
const openDatabase = async (mode) => {
  ...
  return new Promise((resolve, reject) => {
    database = new Database(dbPath, SQLite3[mode], (error) => {
      if (error) {
 ...
 * @return { Promise }
 */
export const all = async (sql: string, tablename: string): Promise<any> => {
  ...
  let callDatabaseAll = () => {
    return new Promise((resolve, reject) => {
      database.all(sql, (error, result) => {
        if (error) {
          reject(
            `in 'database.all' during query in ${tablename} table: ${error}`
          );
        } else {
          resolve(result);
        }
    ...
    return new Promise((resolve, reject) => {
      database.close((error) => {
        if (error) {
          ...
          console.log(
            `Disconnected from material failures database after 'database.all' in ${tablename}`
          );
 ...
 * @return { Promise }
 */
export const get = async (sql, tablename) => {
  let database = await openDatabase("OPEN_READONLY");
  ...
  let callDatabaseGet = () => {
    return new Promise((resolve, reject) => {
      database.get(sql, (error, result) => {
        if (error) {
          reject(
            `in 'database.get' during query in ${tablename} table: ${error}`
          );
        } else {
          resolve(result);
        }
    ...
    return new Promise((resolve, reject) => {
      database.close((error) => {
        if (error) {
          ...
          console.log(
            `Disconnected from material failures database after 'database.get' in ${tablename}`
          );
 ...
 * @return { Promise }
 */
export const update = async (sql, tablename) => {
  ...
  let callDatabaseRun = () => {
    return new Promise((resolve, reject) => {
      database.run(sql, (error) => {
        if (error) {
          reject(
            `in 'database.update' during query in ${tablename} table : ${error}`
          );
        } else {
          resolve();
        }
    ...
    return new Promise((resolve, reject) => {
      database.close((error) => {
        if (error) {
          ...
          console.log(
            `Disconnected from material failures database after 'database.run' in ${tablename}`
          );
